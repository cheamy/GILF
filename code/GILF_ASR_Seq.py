...
class Net_CrossAttention(nn.Module):
    def __init__(self, angular_in, angular_out):
        ...
        self.DeepFeaExtEPI = CascadedBlocksEPI(layer_num, channel, angular_in)
        self.DeepFeaExtSA = CascadedBlocksSA(layer_num, channel, angular_in)
        ...
        self.altblockEPI = self.make_layer_EPI(layer_num=layer_num)
        self.altblockSA = self.make_layer_SA(layer_num=layer_num)
        self.fuse = nn.Sequential(
            nn.Conv3d(in_channels= channel + channel, out_channels=channel, kernel_size=1, stride=1,
                      padding=0, dilation=1),
            nn.LeakyReLU(0.1, inplace=True),
            nn.Conv3d(channel, channel, kernel_size=(1, 3, 3), stride=1, padding=(0, 1, 1), dilation=1))
    #
    ...
    def make_layer_EPI(self, layer_num):
        layers = []
        for i in range(layer_num):
            layers.append(EPIX_Trans(self.angRes, self.channel, self.MHSA_params))
        layers.append(
    ...
    def make_layer_SA(self, layer_num):
        layers = []
        for i in range(layer_num):
            layers.append(SA_Epi_Trans(self.angRes, self.channel, self.MHSA_params))
        layers.append(
    ...
    def forward(self, x):
        ...
        bufferEPI = self.DeepFeaExtEPI(buffer)
        bufferEPI = self.altblockEPI(bufferEPI)
        bufferSA = self.DeepFeaExtSA(bufferEPI)
        buffer = self.fuse(torch.cat((bufferEPI, bufferSA), 1))
        buffer = self.altblockSA(buffer)
...
class SA_Conv(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.SA_fuse(torch.cat((s_out, a_out), 1))
...
class EPI_Conv(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.Epi_fuse(torch.cat((epih_out, epiv_out), 1))
...
class CascadedBlocksEPI(nn.Module):
    '''
    Hierarchical feature fusion
    '''

    def __init__(self, n_blocks, channel, angRes):
        super(CascadedBlocksEPI, self).__init__()
...
class CascadedBlocksSA(nn.Module):
    '''
    Hierarchical feature fusion
    '''

    def __init__(self, n_blocks, channel, angRes):
        super(CascadedBlocksSA, self).__init__()
        self.n_blocks = n_blocks
        body = []
        for i in range(n_blocks):
            body.append(SA_Conv(channel, angRes))
