...
if not os.path.exists('../checkpoint_crossAttention_anyAng_HCI'):
        os.mkdir('../checkpoint_crossAttention_anyAng_HCI')
...
def train(cfg, train_loader, test_Names, test_loaders):
    ...
    for idx_epoch in range(epoch_state, cfg.n_epochs):
        ...
        if idx_epoch % 1 == 0:
            ...
            txtfile = open('../checkpoint_crossAttention_anyAng_HCI/' + cfg.model_name + '_training.txt', 'a')
            ...
            save_ckpt({
                ...
                'loss': loss_list},
                save_path='../checkpoint_crossAttention_anyAng_HCI/', filename=cfg.model_name + '_' + str(cfg.angin) + 'x' + str(cfg.angin)+ 'xSR_' + str(cfg.angout) +
...
def save_ckpt(state, save_path='../checkpoint_crossAttention_anyAng_HCI', filename='checkpoint.pth.tar'):
