...
class EpiFeatureRebuild(nn.Module):
    ...
    #
    #     # 2 x W --> 7 x W
    #     coord = make_coord([self.angRes, self.factor*w]).cuda() \
    #         .unsqueeze(0).expand(epi.shape[0], w * self.angRes_out, 2)
    #     output_epi = self.query_feature(epi, coord, cell=None).permute(0, 2, 1) \
    #         .view(epi.shape[0], -1, self.angRes_out, w)
    #
...
class EpiXTrans(nn.Module):
    ...
    def gen_mask(self, h: int, w: int, maxdisp: int=6):  # when HCI data
...
class SA_Conv(nn.Module):
    def __init__(self, ch, angRes):
        ...
        self.angRes = angRes
        self.spaconv = SpatialConv(angRes, ch)
        ...
        self.Epi_fuse = nn.Sequential(
            nn.Conv3d(in_channels= ch + ch, out_channels=ch, kernel_size=1, stride=1,
                      padding=0, dilation=1),
            nn.LeakyReLU(0.1, inplace=True),
            nn.Conv3d(ch, ch // 2, kernel_size=(1, 3, 3), stride=1, padding=(0, 1, 1), dilation=1))
...
class SpatialConv(nn.Module):
    ...
    def forward(self, fm):
        fm = rearrange(fm, "b c (u v) h w -> b c (h u) (w v)", u=self.angRes, v=self.angRes)
        fm = self.spaconv_s(fm)
        fm = rearrange(fm, "b c (h u) (w v)-> b c (u v) h w", u=self.angRes, v=self.angRes)
        return fm
...
class AngularConv(nn.Module):
    def __init__(self, ch, angRes, AngChannel):
        ...
        self.angRes = angRes
        self.angconv = nn.Sequential(
            nn.Conv2d(ch, AngChannel*self.angRes ** 2, kernel_size=int(angRes), stride=int(angRes), bias=False),
            nn.LeakyReLU(0.2),
            nn.PixelShuffle(self.angRes),
            # nn.PixelShuffle(angRes)
        )
        # self.an = angRes

    def forward(self, fm):
        a_in = rearrange(fm, "b c (u v) h w -> b c (h u) (w v)", u=self.angRes, v=self.angRes)
        out = self.angconv(a_in) # n == angRes * angRes
        out = rearrange(out, "b c (h u) (w v)-> b c (u v) h w", u=self.angRes, v=self.angRes)
...
class EPiConv(nn.Module):
    def __init__(self, channels, angRes, EPIChannel):
        super(EPiConv, self).__init__()
        self.epi_ch = EPIChannel
        self.angRes = angRes
        self.epiconv = nn.Sequential(
            nn.Conv2d(channels, EPIChannel, kernel_size=[1, 1], stride=[1, 1], padding=[0, 0],
                      bias=False),
            nn.LeakyReLU(0.2),
            nn.Conv2d(EPIChannel, EPIChannel, kernel_size=[1, 1], stride=[1, 1], padding=[0, 0],
                      bias=False),
            nn.LeakyReLU(0.2),
            nn.Conv2d(EPIChannel, EPIChannel, kernel_size=[1, 1], stride=[1, 1], padding=[0, 0],
                      bias=False),
            nn.LeakyReLU(0.2),
            nn.Conv2d(EPIChannel, angRes*EPIChannel, kernel_size=[1, 1], stride=[1, 1], padding=[0, 0],
                      bias=False),
        )
        # self.an = angRes
    ...
    def forward(self, fm):
        b, c, u, v, h, w = fm.shape
        epih_in = rearrange(fm, "b c u v h w -> b c (h u) (w v)")
        epih_out = self.epiconv(epih_in)  # (b,self.epi_ch*v, u*h, 1, w)
        epih_out =rearrange(epih_out, "b c (h u) (w v)-> b c (u v) h w", u=self.angRes, v=self.angRes)
        # epih_out = epih_out.reshape(b, self.epi_ch, v, u, h, w).permute(0, 1, 3, 2, 4, 5).reshape(b, self.epi_ch, u * v,
        #                                                                                                   h, w)
        return epih_out
