...
from GILF_ASR_Seq import Net_CrossAttention
...
def parse_args():
    ...
    parser.add_argument('--model_name', type=str, default='GILF_ASR_Seq_HCI')
...
if not os.path.exists('../checkpoint_GILF_ASR_Seq_HCI'):
        os.mkdir('../checkpoint_GILF_ASR_Seq_HCI')
...
def train(cfg, train_loader, test_Names, test_loaders):
    ...
    for idx_epoch in range(epoch_state, cfg.n_epochs):
        ...
        if idx_epoch % 1 == 0:
            ...
            txtfile = open('../checkpoint_GILF_ASR_Seq_HCI/' + cfg.model_name + '_training.txt', 'a')
            ...
            save_ckpt({
                ...
                'loss': loss_list},
                save_path='../checkpoint_GILF_ASR_Seq_HCI/', filename=cfg.model_name + '_' + str(cfg.angin) + 'x' + str(cfg.angin)+ 'xSR_' + str(cfg.angout) +
...
def save_ckpt(state, save_path='../checkpoint_GILF_ASR_Seq_HCI', filename='checkpoint.pth.tar'):
