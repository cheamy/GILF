...
import math
from GILF_ASR_wo_E_s import Net_CrossAttention
...
def parse_args():
    ...
    parser.add_argument('--n_epochs', type=int, default=140, help='number of epochs to train')
    ...
    parser.add_argument('--model_path', type=str, default='../checkpoint_crossAttention_anyAng_HCI/ASRNet_CrossAttention_anyAng_2x2xSR_7x7_epoch_129.pth.tar')

    return parser.parse_args()

if not os.path.exists('../checkpoint_wo_E_s'):
        os.mkdir('../checkpoint_wo_E_s')
...
def train(cfg, train_loader, test_Names, test_loaders):
    ...
    for idx_epoch in range(epoch_state, cfg.n_epochs):
        ...
        if idx_epoch % 1 == 0:
            ...
            txtfile = open('../checkpoint_wo_E_s/' + cfg.model_name + '_training.txt', 'a')
            ...
            if cfg.parallel:
                save_ckpt({
                    ...
                    'loss': loss_list},
                    save_path='../checkpoint_wo_E_s/',
                    filename=cfg.model_name + '_' + str(cfg.angin) + 'x' + str(cfg.angin) + 'xSR_' + str(cfg.angout) +
...
def save_ckpt(state, save_path='../checkpoint_wo_E_s', filename='checkpoint.pth.tar'):
