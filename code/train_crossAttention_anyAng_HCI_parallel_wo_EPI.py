...
import math
from GILF_ASR_wo_EPI import Net_CrossAttention
...
if not os.path.exists('../checkpoint_wo_EPI'):
        os.mkdir('../checkpoint_wo_EPI')
...
def train(cfg, train_loader, test_Names, test_loaders):
    ...
    for idx_epoch in range(epoch_state, cfg.n_epochs):
        ...
        if idx_epoch % 1 == 0:
            ...
            txtfile = open('../checkpoint_wo_EPI/' + cfg.model_name + '_training.txt', 'a')
...
def save_ckpt(state, save_path='../checkpoint_wo_EPI', filename='checkpoint.pth.tar'):
